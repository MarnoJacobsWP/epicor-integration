...
async function contactRepository(fastify) {
  const collection = fastify.mongo.db.collection('contacts');

  if (!fastify.hasDecorator('contactRepository')) {
    fastify.decorate('contactRepository', {
...
export default fp(contactRepository, {
