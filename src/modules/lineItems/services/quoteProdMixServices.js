...
async function quoteProdMixService(fastify, _) {
  ...
  async function syncLineItemsForQuoteWithData(quoteNum, dealId, quoteRecords) {
    ...
    fastify.log.info(`Found ${uniqueRecords.length} unique QuoteProdMix line items for quote ${quoteNum}`);

    const batches = chunkArray(uniqueRecords, BATCH_SIZE);
    const batchResults = [];
    
    for (let i = 0; i < batches.length; i++) {
      fastify.log.info(`Processing QuoteProdMix batch ${i + 1}/${batches.length} (${batches[i].length} line items)...`);
    ...
    fastify.log.info(`QuoteProdMix sync for quote ${quoteNum} complete: ${totalResults.created} created, ${totalResults.updated} updated, ${totalResults.errors} errors, ${totalResults.skipped} skipped`);
